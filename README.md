...
| 17 | [自研 KV/调度的 Hermes 信号提取规格](17-自研KV调度的Hermes信号提取规格.md) | **实现输入规格**：自研全栈(后端与 NVIDIA 不兼容、NeMo+Dynamo 从零重做)前提下，**29 个信号的逐条提取配方**（确切 file:line + W/S/H 提取机制 + 保真度/成本/接缝，两轮回 v0.15.1 源码核验）+ 对报告12–16 的坐标勘误 + 自有全栈相对 NVIDIA 的解锁清单 + host 门控重评估 + 采集层落地路线 |
